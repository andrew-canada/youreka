...
#   x <- listy[[2]]
#   y <- listy[[3]]
# x is the lyme disease rate in 2023, y is the average air pollution concentration
...
multipleRegression <- function(pollutants, startYear, endYear) {
  ...
  library(readxl)
  print("hi")
  hasTick <- read.csv('C:/Users/Sheena/Downloads/7776714/7776714/youreka/Public_Use_Ixodes_County_Table_2024_summary (1).csv')
  print("bye")
...
#         if (determineCounty(precip[i, 3], precip[i, 2])) {
...
#     if (determineCounty(temp[i, 3], temp[i, 2])) {
...
# hi <- lm(influenza ~ pollutionF)
# print(summary(hi))
# print(cor(influenza, pollutionF))
